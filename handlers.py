...
# استيراد الرسائل والقوالب من مسارات المشروع الصحيحة
from messages import WELCOME
from bot.app.services.user_service import get_user_by_telegram_id
from bot.app.services.settings_service import get_float, get_int, get_setting
...
from bot.app.keyboards.inline import daily_reward_kb, support_kb, channel_kb, admin_main_kb
...
async def cmd_start(message: Message, session: AsyncSession):
    ...
    text = WELCOME.format(name=message.from_user.full_name)
    await message.answer(text, parse_mode="HTML", reply_markup=main_menu(is_admin))
...
async def about(message: Message, session: AsyncSession):
    ...
    profit = await get_int(session, "campaign_profit_percent", 30)
    chan = await get_setting(session, "channel_username", "@your_channel")
